...
@WebServlet("/teacher/application-list")
public class ApplicationListServlet extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        User user = (User) req.getSession().getAttribute("user");
        if (user.getType() == 3) {
            List<PassRequest> passRequests = ServiceFactory.getTeacherService().listPassRequests();
            req.setAttribute("passrequests", passRequests);
